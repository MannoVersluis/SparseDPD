...
dma_t* dma_create(
	...
	p_dma_obj->s2mm_bytes_per_beat = 2*hp_bytes;
...
int* dma_set_rcv_buf(dma_t* p_dma_obj, int* p_rcv_buf)
{
    //Assign memory space to receive buffer
	p_rcv_buf = (int*) calloc(p_dma_obj->s2mm_burst_length, p_dma_obj->s2mm_bytes_per_beat);
    if (p_rcv_buf == NULL)
	{
...
{
    return p_dma_obj->s2mm_bytes_per_beat * p_dma_obj->s2mm_burst_length / 4;
}
