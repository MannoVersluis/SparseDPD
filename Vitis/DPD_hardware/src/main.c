...
int main()
{
	...
	status = serial_transfer(p_dma_obj, p_snd_buf, p_rcv_buf);
		...
		if (status == 0)
		{
			printf("Benchmark Successful: TDNN Outputs Correct!!!\r\n");
		} else {
			printf("Benchmark Failed: TDNN Outputs Wrong!!!\r\n");
		}
	#endif
...
int serial_transfer(dma_t* p_dma_obj, short* p_snd_buf, int* p_rcv_buf)
{
	...
	//----------------------------------------------
	XTime tStart, tEnd;
	float step_ops = (float)RNN_PARAM_SIZE;  // Number of operations in the network (per time step)
...
//		cl_out = classification_layer(p_rcv_buf); // #TODO what is this used for?
		...
		if (i - FPGA_DELAY >= 0) {
			...
			{
				printf("	Expected: %d", DPD_hardware_test_gold_rnn[j + rcv_pointer - 2*FPGA_DELAY]);
				err_rnn = *(p_rcv_buf + j) - DPD_hardware_test_gold_rnn[j + rcv_pointer - 2*FPGA_DELAY];
				if (err_rnn != 0)
					total_error += err_rnn;
			}
		}
